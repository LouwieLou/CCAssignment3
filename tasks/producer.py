...
producer = KafkaProducer (bootstrap_servers="129.114.26.21:9092", value_serializer=lambda v:
                                          json.dumps(v).encode('utf-8'), acks=1)  # wait for leader to write to log
...
for i in range (100):
    ...
    #
    producer.send ("utilizations", {'Timestamp': str(i), 'Message': str(contents)})
